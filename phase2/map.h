...
private:
    ...
    const Map *iterator(f first) const
    {
        Map *temp = root;
...
public:
    ...
    class Map *root;
    int cnt;
    Map() : root(nullptr), cnt(0) {}

    // "first" is key and "second" is value
    Map<f, s> *left, *right, *par;
    f first;
    s second;
    int depth;
    ...
    public:
        Iterator(Map<f, s> *_ptr) : ptr(_ptr) {}
        ...
        Iterator &operator++()
        {
            // Implement AVL inorder traversal logic to go to the next element
            if (ptr->right)
            {
                ptr = ptr->right;
                while (ptr->left)
    ...
    const s operator[](int key) const
    {
        return search(key);
    }
